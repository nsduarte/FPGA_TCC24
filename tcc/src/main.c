...
void INICIAR() {
   ...
   GYRO_enableInt1(&sensor, GYRO_INT1_XHIE);    // Threshold interrupt
   GYRO_enableInt2(&sensor,GYRO_REG3_I2_DRDY); // Data Rdy/FIFO interrupt
}
...
//         xil_printf("Z Axis: 0x%04x\n\r", zAxis & 0xFFFF);
         ...
         xil_printf("Temperatura: %d deg C\n\r", temp);
      }
   }
}
